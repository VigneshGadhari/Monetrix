    ...
    db.query(getPasswordQuery, [username], async (err, results) => {
      ...
      if (results.length === 1) {
        ...
        if (passwordMatch) {
          ...
          res.redirect(`/homepage2.html?userId=${results[0].UserID}`);
...
// Add this endpoint to retrieve the user's account balance
// Add this endpoint to retrieve the logged-in user's account balance
app.get('/api/get-balance', (req, res) => {
  // You need to identify the logged-in user; this can be done by including some form of authentication, such as a session or token.
  // For simplicity, let's assume you have a user ID as a query parameter.

  const userId = req.query.userId; // Make sure this user ID is authenticated and obtained securely.

  // Query the database to get the user's account balance
  const getBalanceQuery = 'SELECT AccountBalance FROM UserAccounts WHERE UserID = ?';
  ...
  db.query(getBalanceQuery, [userId], (err, results) => {
      ...
      if (results.length === 1) {
          const balance = results[0].AccountBalance;
          res.json({ balance });
      } else {
